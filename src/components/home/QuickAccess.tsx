...
const QuickAccessCard = memo(function QuickAccessCard({
  ...
  if (isExternal) {
    return (
      <a href={item.href} target={item.target} rel="noopener noreferrer" className={className} style={style}>
        {content}
      </a>
    );
  }
...
export default function QuickAccess() {
  ...
  const updateActiveByViewport = useCallback(() => {
    ...
    cards.forEach((card, idx) => {
      ...
      if (distance < minDistance) { minDistance = distance; nearest = idx; }
    });
  ...
  return (
    ...
    >
      ...
      <div className="container-crfal relative z-10">
        <div className={`mb-10 text-center transition-all duration-500 sm:mb-14 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'}`}>
          ...
          >
            {quickAccessItems.map((item, index) => (
              <div key={item.id} className="w-[286px] flex-shrink-0 snap-center">
                <QuickAccessCard item={item} index={index} isVisible={isVisible} isActive={index === activeIndex} isDark={isDark} isMobile />
              </div>
            ))}
          </div>
          <div className="mt-2 flex justify-center gap-1.5">
            {quickAccessItems.map((item, index) => (
              <div key={item.id} className={`h-1.5 rounded-full transition-all duration-300 ${index === activeIndex ? 'w-5 bg-crfal-blue' : 'w-1.5 bg-neutral-300 dark:bg-neutral-600'}`} />
            ))}
          </div>
        </div>
