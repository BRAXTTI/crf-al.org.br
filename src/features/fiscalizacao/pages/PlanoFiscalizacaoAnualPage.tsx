  ...
  return (
    ...
    >
      ...
      <div className="my-5 space-y-3">
        {etapas.map((etapa, i) => (
          <div key={i} className="flex items-start gap-4 rounded-lg border border-crfal-gray-200 bg-crfal-gray-50 p-4  ">
            <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-crfal-blue-lighter text-crfal-blue  ">
              <etapa.icon className="h-4.5 w-4.5" />
            </span>
            <div>
              <h3 className="text-sm font-bold text-neutral-800 ">{etapa.title}</h3>
              <p className="text-xs leading-relaxed text-crfal-gray-600 ">{etapa.desc}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="my-5 grid gap-3 sm:grid-cols-2">
        {conteudos.map((item, i) => (
          <div key={i} className="flex items-start gap-3 rounded-lg border border-crfal-gray-200 bg-white p-4  ">
            <item.icon className="mt-0.5 h-4 w-4 shrink-0 text-crfal-blue " />
