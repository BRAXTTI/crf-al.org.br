  ...
  return (
    ...
    >
      ...
      <div className="my-5 grid gap-4 sm:grid-cols-3">
        {referencias.map((ref) => (
          <div key={ref.title} className="rounded-xl border border-crfal-gray-200 bg-crfal-gray-50 p-5 transition-colors hover:border-crfal-blue/25   ">
            ...
            <h3 className="mb-1.5 text-sm font-bold text-neutral-800 ">{ref.title}</h3>
            <p className="text-xs leading-relaxed text-crfal-gray-600 ">{ref.desc}</p>
          </div>
        ))}
      </div>
