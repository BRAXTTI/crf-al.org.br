...
function SecaoTitulo({ id, titulo, contador }: { id: string; titulo: string; contador: number }) {
  return (
    <div className="mb-6 flex items-center gap-3">
      <h2 id={id} className="font-display text-xl font-semibold text-neutral-800  md:text-2xl">
        {titulo}
      </h2>
...
function EventoCard({ evento, index }: { evento: Evento; index: number }) {
  ...
  return (
    <article
      className="group relative flex animate-in flex-col overflow-hidden rounded-2xl border border-crfal-gray-200 bg-white shadow-card duration-500 fill-mode-backwards fade-in slide-in-from-bottom-3 transition-all hover:-translate-y-1.5 hover:border-crfal-blue/25 hover:shadow-card-hover  "
    ...
    >
      ...
      <div className="relative flex flex-1 flex-col p-5">
        <h3
          className={`font-display text-lg font-semibold leading-snug transition-colors group-hover:text-crfal-blue  ${
            evento.status === 'cancelado'
              ? 'text-crfal-gray-400 line-through '
              : 'text-neutral-800 '
          }`}
        >
          <Link
        ...
        {evento.descricao && (
          <p className="mt-2 line-clamp-2 text-sm leading-relaxed text-crfal-gray-600 ">
            {evento.descricao}
          </p>
        )}

        <div className="mt-4 space-y-2 border-t border-crfal-gray-100 pt-4 text-sm text-crfal-gray-600  ">
          <p className="flex items-center gap-2">
            <Calendar className="h-4 w-4 shrink-0 text-crfal-blue " />
            <span className="truncate">{formatarData(evento.data, evento.dataFim)}</span>
          </p>
          {evento.local && (
            <p className="flex items-center gap-2">
              {evento.modalidade === 'online' ? (
                <Monitor className="h-4 w-4 shrink-0 text-crfal-blue " />
              ) : (
                <MapPin className="h-4 w-4 shrink-0 text-crfal-blue " />
              )}
          ...
          {evento.publico && (
            <p className="flex items-center gap-2">
              <Building2 className="h-4 w-4 shrink-0 text-crfal-blue " />
      ...
      <div className="relative flex items-center justify-between gap-3 border-t border-crfal-gray-100 px-5 py-3.5 ">
...
function LinhaArquivo({ evento }: { evento: Evento }) {
  ...
  return (
    <Link
      to={`/eventos/${evento.slug}`}
      className={`group relative flex items-center gap-4 px-4 py-3.5 transition-colors hover:bg-crfal-gray-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-crfal-blue-light  sm:px-5 ${
    ...
    >
        ...
        <div
          className={`flex h-12 w-12 shrink-0 flex-col items-center justify-center rounded-xl border ${
            cancelado
              ...
              : 'border-crfal-gray-200 bg-crfal-gray-50 text-crfal-gray-600  '
          }`}
        >
      ...
      <div className="min-w-0 flex-1">
        <p
          className={`truncate text-sm font-semibold transition-colors group-hover:text-crfal-blue  ${
            cancelado
              ? 'text-crfal-gray-400 line-through '
              : 'text-neutral-700 '
          }`}
        ...
        <p className="mt-0.5 truncate text-xs text-crfal-gray-500 ">
      ...
      <span
        className={`hidden shrink-0 items-center gap-1 rounded-full px-2.5 py-1 text-[11px] font-semibold sm:inline-flex ${
          cancelado
            ...
            : 'bg-crfal-gray-100 text-crfal-gray-600  '
        }`}
      >
      ...
      <ChevronRight className="h-4 w-4 shrink-0 text-crfal-gray-300 transition-all group-hover:translate-x-0.5 group-hover:text-crfal-blue " />
    </Link>
  );
}
...
function CardSkeleton() {
  return (
    <div className="overflow-hidden rounded-2xl border border-crfal-gray-200 bg-white  ">
      <div className="aspect-[16/9] animate-pulse bg-crfal-gray-100 " />
      <div className="space-y-3 p-5">
        <div className="h-5 w-3/4 animate-pulse rounded bg-crfal-gray-100 " />
        <div className="h-3.5 w-full animate-pulse rounded bg-crfal-gray-100 " />
        <div className="h-3.5 w-2/3 animate-pulse rounded bg-crfal-gray-100 " />
        <div className="h-10 w-full animate-pulse rounded-lg bg-crfal-gray-100 " />
      </div>
    </div>
  );
}
      ...
      <main className="container-crfal py-8 md:py-12">
        {isLoading ? (
          ...
          <div className="mx-auto max-w-xl rounded-2xl border border-red-200 bg-white p-8 text-center  ">
            <p className="mb-4 text-red-600 ">Não foi possível carregar os eventos agora.</p>
            <p className="mb-6 text-sm text-crfal-gray-500 ">
          ...
          <div className="rounded-2xl border border-dashed border-crfal-gray-300 bg-white py-16 text-center  ">
            <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-crfal-blue-lighter ">
              <CalendarDays className="h-8 w-8 text-crfal-blue " />
            </div>
            <h2 className="font-display text-xl font-semibold text-neutral-800 ">
              Nenhum evento cadastrado
            </h2>
            <p className="mt-2 text-crfal-gray-500 ">
          ...
          <div className="space-y-10">
            ...
            {destaque && (
              <section aria-labelledby="destaque-titulo">
                ...
                <article className="group relative grid overflow-hidden rounded-2xl border border-crfal-gray-200 bg-white shadow-card transition-all duration-500 hover:shadow-card-hover   md:grid-cols-2">
                  ...
                  <div className="relative flex flex-col justify-center gap-4 p-6 md:p-10">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <span className="inline-flex items-center gap-1.5 rounded-full bg-crfal-blue-lighter px-2.5 py-1 text-[11px] font-semibold text-crfal-blue  ">
                        {(() => {
                    ...
                    <h3 className="font-display text-2xl font-semibold leading-tight text-neutral-800  md:text-3xl">
                      <Link
                    ...
                    {destaque.descricao && (
                      <p className="line-clamp-3 text-sm leading-relaxed text-crfal-gray-600  md:text-base">
                        {destaque.descricao}
                      </p>
                    )}

                    <div className="space-y-1.5 text-sm text-crfal-gray-600 ">
                      <p className="flex items-center gap-2">
                        <Calendar className="h-4 w-4 shrink-0 text-crfal-blue " />
            ...
            <div className="sticky top-[72px] z-30 rounded-2xl border border-crfal-gray-200/80 bg-white/85 p-3 shadow-card backdrop-blur-lg   sm:p-4">
              <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
                <div className="relative lg:max-w-xs lg:flex-1">
                  ...
                  <input
                    ...
                    className="w-full min-h-[44px] rounded-full border border-crfal-gray-200 bg-white py-2.5 pl-10 pr-10 text-sm text-neutral-800 transition-colors placeholder:text-crfal-gray-400 focus:border-crfal-blue-light focus:outline-none focus:ring-2 focus:ring-crfal-blue-light/40   "
                  />
                  {busca && (
                    <button
                      onClick={() => setBusca('')}
                    ...
                    <button
                      ...
                      className={`inline-flex min-h-[44px] shrink-0 snap-start items-center gap-1.5 rounded-full px-4 py-2 text-sm font-medium transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-crfal-blue-light ${
                        filtroStatus === filtro.value
                          ...
                          : 'bg-crfal-gray-100 text-crfal-gray-600 hover:bg-crfal-gray-200 hover:text-crfal-blue   '
                      }`}
                    >
                      {filtro.label}
                      <span
              ...
              {!emLanding && (
                <p className="mt-3 flex items-center gap-2 border-t border-crfal-gray-100 pt-3 text-xs text-crfal-gray-500  ">
                  {totalResultados} resultado{totalResultados !== 1 ? 's' : ''} encontrado
                  {totalResultados !== 1 ? 's' : ''}
                  <button
                    onClick={limparFiltros}
                    className="ml-auto inline-flex min-h-[32px] items-center gap-1 font-semibold text-crfal-blue transition-colors hover:underline "
                  >
              ...
              <>
                ...
                {arquivoFiltrado.length > 0 && (
                  <section aria-labelledby="arquivo-titulo">
                    ...
                    <div className="divide-y divide-crfal-gray-100 overflow-hidden rounded-2xl border border-crfal-gray-200 bg-white   ">
                    ...
                    {arquivoFiltrado.length > 4 && (
                      <div className="mt-4 text-center">
                        <button
                          ...
                          className="inline-flex min-h-[44px] items-center gap-2 rounded-full border border-crfal-gray-200 bg-white px-5 py-2.5 text-sm font-semibold text-crfal-gray-600 transition-all hover:border-crfal-blue/40 hover:text-crfal-blue    "
                        >
                          {mostraArquivo
                            ? 'Mostrar menos'
